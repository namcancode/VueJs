    ...
    <div class="row">
        <div class="col-xl-6 offset-xl-3">
            ...
            <Card title="Thông tin ứng viên" v-loading="loading.info">
                <div slot="header">
                    <h4 class="card-title">
                        <el-button icon="ti-arrow-left" circle type="info" @click="$router.push({name: 'javiet'})" /> Thông tin ứng viên
                    </h4>
                </div>
                <div class="row">
                    ...
                    <Form-Element title="Cập nhật" size="col-4">{{applicant.updatedAt | date('HH:mm dd/MM')}}</Form-Element>
...
export default class JavietDetail extends Vue {
  ...
  create() {
    ...
    request
      .post(this.apiUrl, this.newEngage)
      .then(rs => {
  ...
  update(index: number) {
    ...
    request
      .put(`${this.apiUrl}/${engage._id}`, engage)
      .then(rs => {
